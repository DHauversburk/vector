...
export const OfflineProvider = ({ children }: { children: ReactNode }) => {
      ...
      for (const req of queue) {
        ...
        if (req.retryCount > 0 && req.lastAttempt) {
          ...
          if (now - req.lastAttempt < waitTime) {
            logger.debug(
              'Sync',
              `Backoff: Skipping ${req.operationName} (Retry #${req.retryCount} pending)`,
            )
            continue
          }
        }
        ...
        try {
          logger.debug(
            'Sync',
            `Processing ${req.operationName} (Attempt ${req.retryCount + 1})...`,
            req.body,
          )
  ...
  return (
    <OfflineContext.Provider
      value={{ isOnline, pendingCount, executeMutation, syncNow: processQueue }}
    >
      {children}
    </OfflineContext.Provider>
  )
