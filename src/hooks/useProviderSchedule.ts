...
export function useProviderSchedule() {
    ...
    const { 
        viewMode, setViewMode, currentDate, setCurrentDate, navigate 
    } = useScheduleState();

    // 2. Data Synchronization (Fetching, Auto-refresh, history)
    const {
        appointments, setAppointments, loading,
        patientHistory, historyLoading, loadPatientHistory,
        updates, showUpdates, setShowUpdates, clearUpdates,
        loadAppointments
