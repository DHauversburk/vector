...
export const supabaseAdmin: IAdminActions = {
  getMembers: async (search?: string): Promise<PublicUser[]> => {
    let query = supabase
      .from('users')
      .select('*, appointments!member_id(count)')
      .eq('role', 'member')
    if (search) {
  ...
  adminResetUserSecurity: async (userId: string): Promise<boolean> => {
    ...
    const { error } = await supabase.from('user_pins').delete().eq('user_id', userId)
  ...
  getAuditLogs: async (
    filters: { type?: string; severity?: string; limit?: number } = {},
  ): Promise<AuditLog[]> => {
  ...
  pruneInactiveUsers: async (days: number): Promise<number> => {
    const { data, error } = await supabase.rpc('admin_prune_unused_accounts', {
      days_inactive: days,
    })
    if (error) throw error
    return data as number
