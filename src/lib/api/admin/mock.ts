...
export const mockAdmin: IAdminActions = {
  getMembers: async (search?: string): Promise<PublicUser[]> => {
    ...
    if (search) {
      return mockMembers.filter(
        (m) => m.token_alias.includes(search.toUpperCase()) || m.id.includes(search),
      ) as PublicUser[]
    }
  ...
  logEvent: async (
    _type: string,
    _description: string,
    _severity: 'INFO' | 'WARN' | 'ERROR' | 'CRITICAL' = 'INFO',
  ...
  getAuditLogs: async (
    _filters: { type?: string; severity?: string; limit?: number } = {},
  ): Promise<AuditLog[]> => {
    return []
  },
