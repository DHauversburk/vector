...
export interface IAppointmentActions {
    ...
    submitFeedback(appointmentId: string, rating: number, comment?: string): Promise<{ success: boolean }>;
    ...
    createAppointment(appt: Omit<Appointment, 'id' | 'created_at' | 'provider' | 'member'>): Promise<Appointment>;
}
...
export interface IProviderActions {
    ...
    getProviderSchedule(providerId: string, startDate: string, endDate: string): Promise<Appointment[]>;
    generateSlots(startDate: string, endDate: string, startTime: string, endTime: string, duration: number, breakMinutes: number, days: number[], isBlock?: boolean, notes?: string | null): Promise<unknown>;
    clearSchedule(startDate: string, endDate: string, includeBooked?: boolean): Promise<{ count?: number; deleted?: number; success?: boolean; method?: string }>;
    toggleSlotBlock(slotId: string, isBlocked: boolean, notes?: string | null): Promise<Appointment>;
    getAnalytics(): Promise<{ appointments: Appointment[]; feedback: Feedback[]; noteStats: NoteStatistics[] }>;
    getProviderResources(providerId: string): Promise<ProviderResource[]>;
    getMyResources(): Promise<ProviderResource[]>;
    addResource(resource: Omit<ProviderResource, 'id' | 'provider_id' | 'created_at'>): Promise<ProviderResource>;
...
export interface IAdminActions {
    ...
    logEvent(type: string, description: string, severity?: 'INFO' | 'WARN' | 'ERROR' | 'CRITICAL', metadata?: Record<string, unknown>): Promise<void>;
    getAuditLogs(filters?: { type?: string; severity?: string, limit?: number }): Promise<AuditLog[]>;
    getSystemStats(): Promise<SystemStats>;
    adminCreateUser(email: string, pass: string, token: string, role: string, serviceType: string): Promise<string>;
...
export interface IInteractionActions {
    addEncounterNote(note: Omit<EncounterNote, 'id' | 'created_at'> & { id?: string }): Promise<EncounterNote>;
    ...
    bulkArchiveNotes(beforeDate: string, providerId?: string): Promise<{ archivedCount: number; notes: EncounterNote[] }>;
    createHelpRequest(request: Omit<HelpRequest, 'id' | 'member_id' | 'status' | 'created_at'> & { id?: string }): Promise<HelpRequest>;
    getMyHelpRequests(): Promise<HelpRequest[]>;
    getPendingHelpRequests(): Promise<HelpRequest[]>;
    updateHelpRequestStatus(requestId: string, status: HelpRequest['status']): Promise<HelpRequest | null>;
    ...
    joinWaitlist(providerId: string, serviceType: string, note?: string, preferredDays?: number[]): Promise<WaitlistEntry>;
