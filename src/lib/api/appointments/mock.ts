...
export const mockAppointments: IAppointmentActions = {
    getMyAppointments: async (startDate?: string, endDate?: string): Promise<Appointment[]> => {
        ...
        return myAppts.map(a => {
            ...
            let alias = 'UNKNOWN';
            let st = 'PRIMARY';
            if (a.provider_id.includes('jameson')) { alias = 'Dr. Jameson'; st = 'MH_GREEN'; }
            else if (a.provider_id.includes('smith')) { alias = 'Dr. Smith'; st = 'PRIMARY_BLUE'; }
            else if (a.provider_id.includes('taylor')) { alias = 'Dr. Taylor'; st = 'PT_GOLD'; }
