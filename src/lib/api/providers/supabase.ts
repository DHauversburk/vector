...
export const supabaseProviders: IProviderActions = {
  ...
  getAnalytics: async (): Promise<{
    ...
    return {
      ...
      feedback: fbError ? [] : (feedback as unknown as Feedback[]) || [],
      noteStats,
    }
  },
  ...
  > => {
    ...
    if (aptError) throw aptError
    const providerIds = [
      ...new Set((appointments || []).map((a: { provider_id: string }) => a.provider_id)),
    ] as string[]
    ...
    return providerIds
      .map((pid) => ({
        provider: pid,
        resources: ((resources || []) as unknown as ProviderResource[]).filter(
          (r) => r.provider_id === pid,
        ),
      }))
