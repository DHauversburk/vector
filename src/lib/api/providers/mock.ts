...
export const mockProviders: IProviderActions = {
  getProviders: async (): Promise<ProviderProfile[]> => {
    return [
      {
        id: 'mock-provider-jameson',
        token_alias: 'Dr. Jameson',
        role: 'provider',
        service_type: 'MH_GREEN',
      },
      {
        id: 'mock-provider-smith',
        token_alias: 'Dr. Smith',
        role: 'provider',
        service_type: 'PRIMARY_BLUE',
      },
      {
        id: 'mock-provider-taylor',
        token_alias: 'Dr. Taylor',
        role: 'provider',
        service_type: 'PT_GOLD',
      },
    ]
  },
  ...
  getProviderSchedule: async (
    ...
    return mockStore.appointments
      .filter(
        (a) => a.provider_id === providerId && a.start_time >= startDate && a.start_time <= endDate,
      )
  ...
  updateResource: async (
    id: string,
    updates: Partial<ProviderResource>,
  ...
  getAvailableResources: async (): Promise<
    { provider: string; resources: ProviderResource[] }[]
  > => {
    const {
      data: { user },
    } = await supabase.auth.getUser()
