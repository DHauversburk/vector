        ...
        <div className="flex h-14 items-center px-4 md:px-6 gap-4">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-3 px-3 py-1.5 rounded-md border text-[10px] font-black tracking-widest transition-all shadow-sm ${
                actionContext === 'block'
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 transition-transform ${isContextOpen ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {isContextOpen && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="p-1">
                  <div className="px-3 py-1.5 text-[10px] text-slate-400 font-black uppercase tracking-widest">
                    Control Context
                  </div>
                  <button
                    onClick={() => {
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="hidden md:flex text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white font-bold text-xs uppercase tracking-wider hover:bg-slate-100 dark:hover:bg-slate-800"
          >
        ...
        {mobileMenuOpen && (
          <div className="absolute top-14 left-0 w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-xl md:hidden flex flex-col p-4 gap-2 animate-in slide-in-from-top-2 z-50">
            <Button
              ...
              className="justify-start h-10 text-xs font-black uppercase tracking-wider"
            >
              {' '}
              <Grid className="w-4 h-4 mr-3" /> Master Schedule{' '}
            </Button>
            <Button
              variant={currentView === 'tokens' ? 'secondary' : 'ghost'}
              size="sm"
              onClick={() => setCurrentView('tokens')}
              className="justify-start h-10 text-xs font-black uppercase tracking-wider"
            >
              {' '}
              <FileText className="w-4 h-4 mr-3" /> Tokens{' '}
            </Button>
            <Button
            ...
            >
              {' '}
              <AlertCircle className="w-4 h-4 mr-3" /> Audit Logs{' '}
            </Button>
            <Button
            ...
            >
              {' '}
              <Database className="w-4 h-4 mr-3" /> System Hygiene{' '}
            </Button>
            <Button
            ...
            >
              {' '}
              <MessageSquare className="w-4 h-4 mr-3" /> Beta Feedback{' '}
            </Button>
            ...
            <Button
              ...
              size="sm"
              onClick={signOut}
              className="justify-start h-10 text-xs font-black uppercase tracking-wider text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20"
            >
              {' '}
      ...
      <main className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-12 gap-6 max-w-[1600px] mx-auto">
        ...
        <div className="md:col-span-3 lg:col-span-2 space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 space-y-1 shadow-sm transition-colors">
            <div className="text-[10px] uppercase text-slate-400 font-black mb-3 px-2 tracking-widest">
              Console
            </div>
            {(['schedule', 'tokens', 'logs', 'maintenance', 'feedback'] as DashboardView[]).map(
              (v) => (
                <button
                  key={v}
