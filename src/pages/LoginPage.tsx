...
export default function LoginPage() {
    ...
    return (
        <LoginBackground isLoaded={isLoaded}>
            ...
            {/* MAIN CONTENT */}
            <div className={`w-full max-w-md relative z-10 transition-all duration-700 ${isLoaded('logo') ? 'opacity-100' : 'opacity-0'}`}>
                        ...
                        <div className="relative bg-slate-900/80 backdrop-blur-2xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl">
                            {stage === 'auth' ? (
                                <AuthForm
                                    ...
                                    handleLogin={handleLogin} handleBiometric={handleBiometric}
                                    loading={loading} error={error}
                                    bootComplete={bootComplete} setShowTokenHelp={setShowTokenHelp}
                                    tokenInputRef={tokenInputRef} isLoading={isLoading}
                                    isLoaded={isLoaded} currentLoadingText={currentLoadingText}
                                />
                            ) : stage === 'pin' ? (
                                <PinVerification error={error} pinLoading={pinLoading} onComplete={handlePinComplete} />
                            ) : stage === 'reset' ? (
                                <ResetFlow error={error} handleReset={handleReset} onCancel={() => setStage('auth')} />
                            ) : (
                ...
                {/* Footer and status */}
                <div className={`mt-8 text-center space-y-3 transition-opacity duration-700 ${isLoaded('footer') ? 'opacity-100' : 'opacity-0'}`}>
                    {stage === 'pin' && (
                        <button
                    ...
                    <div className={`inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full ${IS_MOCK ? 'text-amber-400 bg-amber-950/50 border border-amber-900/50' : 'text-emerald-400 bg-emerald-950/50 border border-emerald-900/50'}`}>
                    ...
                    {IS_MOCK && bootComplete && (
                        <button
                            onClick={() => { if (confirm('Reset all demo data?')) { api.mockStore.reset(); window.location.reload(); } }}
