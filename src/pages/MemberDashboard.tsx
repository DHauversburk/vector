...
export default function MemberDashboard() {
    const {
        user, signOut,
        appointments, providers, myWaitlist, loading,
        ...
        >
                                                ...
                                                upcomingAppts.length > 0 ? (
                                                    upcomingAppts.map(appt => (
                                                        <AppointmentRow 
                                                            ...
                                                            onFeedback={(id: string) => { setFeedbackApptId(id); setFeedbackOpen(true); }}
                                                        />
                                                    ))
                                                ) : (
                                                    <div className="p-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                                                        ...
                                                        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">No Active Missions</p>
                                                    </div>
                                                )
                                            ) : (
                                                appointments.filter(a => new Date(a.start_time) < new Date() || a.status === 'cancelled').length > 0 ? (
                                                    appointments.filter(a => new Date(a.start_time) < new Date() || a.status === 'cancelled')
                                                    ...
                                                    <p className="text-center py-10 text-slate-500 text-xs">NO MISSION HISTORY RECORDED</p>
                                                )
                                            )}
                                        </ErrorBoundary>
                                    </div>
                                ...
                                <ErrorBoundary name="MemberSidebar">
                                    <div className="space-y-6">
                                        <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-indigo-500/10 border border-indigo-500/20">
                                            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-4">Operations Center</h3>
                                            <div className="space-y-3">
                                                ...
                                                >
                                                    <span className="text-xs font-black uppercase tracking-widest">New Mission</span>
            ...
            {bookingOpen && (
                <BookingConsole 
                    ...
                    onRequestWaitlist={(providerId) => { setWaitlistProviderId(providerId); setWaitlistOpen(true); }}
            ...
            <Suspense fallback={null}>
                {helpModalOpen && <HelpRequestModal isOpen={helpModalOpen} onClose={() => setHelpModalOpen(false)} />}
                {waitlistOpen && (
                    <WaitlistModal 
                        isOpen={waitlistOpen} 
                        onClose={() => setWaitlistOpen(false)} 
                        providerId={waitlistProviderId} 
                        serviceType={providers.find(p => p.id === waitlistProviderId)?.service_type || 'General'}
                    />
                )}
                {feedbackOpen && feedbackApptId && (
                    <FeedbackModal
                        isOpen={feedbackOpen}
                        onClose={() => { setFeedbackOpen(false); setFeedbackApptId(null); }}
