...
import { Fingerprint, ShieldAlert, Loader2, ChevronRight } from 'lucide-react'
      ...
      <div className="relative w-full max-w-sm flex flex-col items-center">
        ...
        <div className="relative mb-8 group">
          ...
          <div
            className={cn(
              ...
              isAuthenticating
                ? 'border-indigo-400 shadow-indigo-500/40 scale-105'
                : 'border-slate-800',
        ...
        <div className="w-full space-y-4">
          ...
          <Button
            variant="ghost"
            onClick={onSignOut}
            className="w-full text-slate-500 hover:text-rose-400 text-sm hover:bg-rose-500/5"
          >
            Sign out
          </Button>
        </div>
        ...
        {error && (
          <p className="mt-8 text-sm text-rose-500 animate-in fade-in slide-in-from-bottom-1 text-center">
            {error === 'AUTHENTICATION_REJECTED'
              ? "We couldn't verify you. Try again."
              : 'Sensor error. Try manual sign-in?'}
          </p>
        )}
      </div>
    </div>
