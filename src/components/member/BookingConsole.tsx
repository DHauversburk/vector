...
export function BookingConsole({
  ...
  const handleSlotBooking = async (slotId: string) => {
    ...
    if (slot) {
      const hasAppointmentToday = appointments.some(
        (appt) =>
          isSameDay(parseISO(appt.start_time), parseISO(slot.start_time)) &&
  ...
  return (
    <div
      className={`p-6 bg-white dark:bg-slate-900 border rounded-lg shadow-xl space-y-6 animate-in slide-in-from-top-4 duration-300 ${isRescheduling ? 'border-amber-200 dark:border-amber-900 ring-1 ring-amber-100 dark:ring-amber-900/30' : 'border-slate-200 dark:border-slate-800'}`}
    >
      <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-4">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded ${isRescheduling ? 'bg-amber-50 dark:bg-amber-950 text-amber-600' : 'bg-indigo-50 dark:bg-indigo-950 text-indigo-600'}`}
          >
            <Calendar className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">
              {isRescheduling ? 'Reschedule Appointment' : 'Clinical Service Enrollment'}
            </h3>
        ...
        {isRescheduling && (
          <button
            ...
            className="text-[9px] font-black uppercase text-amber-600 hover:text-amber-700 bg-amber-50 dark:bg-amber-950 px-3 py-1 rounded"
          >
            Cancel Reschedule
          </button>
        )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          >
            ...
            <option value="Administrative / Documentation Request">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-white/70 mt-0.5">
                      First available:{' '}
                      {format(parseISO(firstAvailableSlot.start_time), 'EEE, MMM d @ HH:mm')}
                    </p>
                  </div>
                </div>
                <button
