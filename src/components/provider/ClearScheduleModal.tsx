...
export function ClearScheduleModal({ isOpen, onClose, onSuccess }: ClearScheduleModalProps) {
  ...
  const handleClear = async (e: React.FormEvent) => {
    ...
    if (
      cleanBooked &&
      !confirm(
        'DANGER: You are about to delete BOOKED appointments as well. This will cancel patients without notification. Are you sure?',
      )
    ) {
      return
    }
      ...
      >
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-600 dark:text-slate-300 font-medium">
            Define the range to clear. By default, this only removes{' '}
            <span className="text-indigo-600 font-bold">OPEN</span> slots.
          </p>
          <form
            id="clear-form"
            onSubmit={handleClear}
            className="space-y-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-lg"
          >
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <label
                htmlFor="cleanBooked"
                className="text-xs font-bold text-red-600 select-none cursor-pointer"
              >
