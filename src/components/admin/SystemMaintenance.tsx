...
export const SystemMaintenance = () => {
  ...
  const fixDuplicates = async () => {
    if (
      !confirm(
        'WARNING: This will delete duplicate user records, keeping the oldest creation. Appointments linked to deleted users may be lost or orphaned. Continue?',
      )
    )
      return
    setLoading(true)
  ...
  const handleWipeMockData = async () => {
    if (
      !confirm(
        'EXTREME DANGER: This will wipe ALL mock appointments, settings, and local changes. \n\nThis is irreversible. Are you sure?',
      )
    )
      return
    try {
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded p-4 space-y-3">
            ...
            <Button
              onClick={async () => {
                ...
                if (!confirm(`CONFIRM: Delete inactive MEMBER accounts older than ${days} days?`))
                  return
                try {
                  setLoading(true)
