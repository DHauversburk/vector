...
export default function TokenGenerator({ isProvider = false }: TokenGeneratorProps) {
  ...
  const processedDirectory = directory
    .filter((u) => {
      ...
      if (searchText) {
        ...
        return (
          (u.token_alias || '').toLowerCase().includes(low) ||
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Permanently delete ${selectedIds.size} selected users? This cannot be undone.`))
      return
    setLoadingDir(true)
    try {
  ...
  const loadDirectory = async () => {
    setLoadingDir(true)
    try {
      const [members, providers] = await Promise.all([api.getMembers(), api.getProviders()])
  ...
  const handleDelete = async (userId: string, alias: string) => {
    if (!confirm(`DANGER: Permanently delete user ${alias || userId}? This cannot be undone.`))
      return
    try {
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            ...
            <div className="md:col-span-2">
              ...
              >
                {isGenerating
                  ? `Provisioning (${statusParams.success}/${statusParams.total})...`
                  : 'Generate Identities'}
              </Button>
            </div>
          </div>
            ...
            <>
              <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-4 bg-white dark:bg-slate-900 border-t border-t-slate-100 dark:border-t-slate-800">
                ...
                <select
                  ...
                  value={sortBy}
                  onChange={(e) =>
                    setSortBy(e.target.value as 'created_desc' | 'alias_asc' | 'role')
                  }
                >
                ...
                {selectedIds.size > 0 && (
                  <Button
                    ...
                    className="bg-red-600 hover:bg-red-700 text-white h-8 text-[10px] uppercase font-black tracking-widest border-none"
                  >
                ...
                <table className="w-full text-left">
                    ...
                    <tr>
                      <th className="px-4 py-3 w-8">
                        <input
                          type="checkbox"
                          checked={
                            selectedIds.size === processedDirectory.length &&
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {processedDirectory.map((user) => (
                      <tr
                        key={user.id}
                        className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${selectedIds.has(user.id) ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${user.role === 'provider' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600'}`}
                          >
                            {user.role}
                          </span>
                        </td>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                        />
                        <div className="font-mono text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight truncate max-w-[150px]">
                          {user.token_alias || user.email?.split('@')[0] || 'UNKNOWN'}
                        </div>
                      </div>
                      <span
                        className={cn(
                    ...
                    <div className="flex items-center justify-between text-[10px] font-bold text-slate-500 uppercase tracking-tighter">
                      <div className="flex flex-col">
                        <span className="text-slate-400 text-[8px] mb-0.5">SERVICE UNIT</span>
                        {user.service_type || 'N/A'}
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-slate-400 text-[8px] mb-0.5">ID FRAGMENT</span>
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-[9px] text-slate-400 font-mono">
                        {user.created_at ? format(parseISO(user.created_at), 'MM.dd.yy') : '-'}
                      </span>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleResetPin(user.id, user.token_alias || '')}
                          className="text-[9px] font-black uppercase text-amber-500 bg-amber-50 dark:bg-amber-900/10 px-3 py-1.5 rounded active:scale-95 transition-all"
                        >
                          Reset PIN
                        </button>
                        <button
          ...
          <div className="flex justify-between items-center print:hidden border-b border-slate-200 pb-4">
            <div className="flex items-center gap-4">
              <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">
                Active Batch: {lastBatch.id.toUpperCase()}
              </h3>
            ...
            <Button
              ...
              size="sm"
              onClick={handlePrint}
              className="border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-[10px] uppercase tracking-widest"
            >
                  ...
                  <div className="w-full text-left mt-3 pt-2 border-t border-slate-100 flex items-center justify-between">
                    <span
                      className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${
                        lastBatch.serviceType === 'PT_BLUE'
                          ? 'bg-blue-50 text-blue-700'
                          : lastBatch.serviceType === 'MH_GREEN'
