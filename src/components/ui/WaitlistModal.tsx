...
export function WaitlistModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const entry = await executeMutation('JOIN_WAITLIST', {
        providerId,
        serviceType,
        note,
        preferredDays,
      ...
      toast.success(
        isOnline ? 'Joined waitlist successfully!' : 'Waitlist request queued for sync.',
      )
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-amber-950/30 border border-amber-500/20 rounded-lg p-3 flex gap-3">
            ...
            <p className="text-xs text-amber-200">
              You'll receive a notification if an appointment becomes available matching your
              preferences.
            </p>
          </div>
          ...
          {error && (
            <div
              role="alert"
              className="p-3 bg-red-950/50 border border-red-500/50 text-red-400 text-xs rounded-lg"
            >
              {error}
            </div>
          )}
