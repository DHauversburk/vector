...
export default function CommandPalette() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('vector-toggle-command-palette', handleToggleEvent)

    return () => {
      window.removeEventListener('keydown', handleKeyDown)
      window.removeEventListener('vector-toggle-command-palette', handleToggleEvent)
    }
  }, [])
  ...
  const commands: CommandItem[] = [
    ...
    },
    {
      ...
      category: 'Navigation',
      action: () =>
        window.dispatchEvent(new CustomEvent('vector-navigate', { detail: 'security' })),
    },
    {
      id: 'theme',
      label: 'Toggle Theme',
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[8vh] sm:pt-[20vh] px-4">
      {/* Backdrop */}
      <div
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center px-4 py-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-1">
            <kbd className="px-2 py-0.5 text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
              ESC
            </kbd>
          </div>
        </div>
              ...
              {filteredCommands.map((cmd, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-1.5 rounded-md ${index === selectedIndex ? 'bg-indigo-100 dark:bg-indigo-500/20' : 'bg-slate-100 dark:bg-slate-800'}`}
                    >
                      {cmd.icon}
                    </div>
        ...
        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[10px] text-slate-400">
          <div className="flex gap-4">
            <span>
              <strong className="text-slate-600 dark:text-slate-400">↑↓</strong> Navigate
            </span>
            <span>
              <strong className="text-slate-600 dark:text-slate-400">↵</strong> Select
            </span>
          </div>
